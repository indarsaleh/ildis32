...
/**
 * DokumenJdihSearch represents the model behind the search form of `backend\models\DokumenJdih`.
 */
class DokumenJdihSearch extends DokumenJdih
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        $query = DokumenJdih::find()->where(['is_publish'=>0]);

        // add conditions that should always apply here

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
            'sort' => ['defaultOrder' => ['is_publish' => SORT_ASC]]
        ]);
